...
class Sprite:
    ...
    def collide(self, objet):
        return dist(self.pos, objet.get_pos()) <= self.radius + objet.get_radius()
...
def rock_spawner():
    global a_rock, rock_group, lives
...
def process_sprite_group(canvas, rks):
    new = set(rks)
    for n in new:
        if n.update():
            rks.discard(n)
